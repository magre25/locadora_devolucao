...
-- procedimento de devolução de carros
create or replace function proc_devolucao(placa_car varchar)
returns integer
as $$
declare 
	situacao_car carro.situacao%TYPE;
begin
	select situacao into situacao_car
	from carro 
	where placa = placa_car;
	
	if(situacao_car = 'locado') then
		update carro set situacao = 'disponivel'
		where placa = placa_car;
		
		update reserva set situacao = 'Concluido'
		where placa = placa_car;
...
-- TESTE
select proc_devolucao('MUY-5853');